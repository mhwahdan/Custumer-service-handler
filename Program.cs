...
var connectionString = builder.Configuration.GetConnectionString("Server=DESKTOP-P16LG38\\LOCAL_SERVER;Database=POC-updated;Trusted_Connection=True;MultipleActiveResultSets=true;");

builder.Services.AddDbContext<PocContext>(options =>
    options.UseSqlServer("Server=DESKTOP-P16LG38\\LOCAL_SERVER;Database=POC-updated;Trusted_Connection=True;"));
...
builder.Services.AddIdentity<POC.Models.Employee, IdentityRole>(options => options.SignIn.RequireConfirmedAccount = true).AddDefaultUI().AddDefaultTokenProviders().AddEntityFrameworkStores<PocContext>();
